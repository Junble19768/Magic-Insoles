...
> `backend/` 为已弃用测试桩，见 `backend/DEPRECATED.md` 与 ADR-0003。
...
| `/api/` | FastAPI (`backend_prod`) | uvicorn `127.0.0.1:8000`，Nginx 反代 |
| `/health` | 健康检查 | 反代至 uvicorn `/health` |
| 设备 TCP | `backend_prod` tcp_ingest | `0.0.0.0:9000`（安全组按需放行） |

### 一键部署

```powershell
# 本地 Windows（build + scp + 重启 API）
.\deploy\deploy.ps1
```

ECS 路径：`/var/www/magic-insoles/backend_prod/`（venv 在 `/var/www/magic-insoles/venv`）。

### 部署步骤摘要（手动等价）

```bash
cd frontend && npm run build
# dist/ → ECS:/var/www/insoles/dist
# backend_prod/ → ECS:/var/www/magic-insoles/backend_prod/
# 参考 deploy/magic-insoles-api.service、deploy/nginx.conf
```

安全组放行入站 TCP 80；设备接入时放行 TCP 9000。
