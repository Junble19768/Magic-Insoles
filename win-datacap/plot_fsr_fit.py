...
分压电路：Vcc -- FSR -- 节点 -- R_fixed -- GND
fsr_00 为 R_fixed 两端电压；由分压公式反算 FSR 阻值。

拟合模型（R 为 FSR 阻值，F 为参考压力绝对值）：
  指数：R = a * exp(b * F)
  幂函数：R = a * F^b
  倒数：R = a / F + c
"""
...
# ── 硬件参数 ──────────────────────────────────────────────
VCC = 3.3  # ADC 参考电压 (V)，与 USB-DAQ 一致
R_FIXED_OHM = 35_000.0  # 分压固定电阻 (Ω)

DEFAULT_CSV = Path(__file__).resolve().parent / "record" / "20260706_212154.csv"
FSR_COL = "fsr_00"
FORCE_COL = "force_ch0"
FORCE_BIN_N = 0.5  # 压力分箱宽度 (N)，取每箱中位数降噪
...
def load_and_aggregate(csv_path: Path) -> tuple[np.ndarray, np.ndarray]:
    """读取 CSV，按压力分箱聚合，返回 (force_N, resistance_ohm)。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FSR 压力-阻值标定曲线拟合")
    parser.add_argument(
        "csv",
        nargs="?",
